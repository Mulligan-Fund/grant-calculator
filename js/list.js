  ...
  gc.Views.listView = Backbone.View.extend({
      ...
      this.collection.getData(
        ...
        function(data) {
          if (data.length < 1) {
            ...
            t.html("<a class='button' href='./profile'>Create profile</a>");
          }
    ...
    getObjectList: function(context) {
      ...
      this.collection.getObjectData({ list: true }, function(data) {
        ...
        var count = 0;
        for (var t in data) {
          console.log("Checking data", data[t]);
          if (typeof data[t].delete != "undefined" && data[t].delete == true)
            continue;
          $(".objlist ul").append("<li>" + data[t].name + "</li>");
          count += 1;
          if (count >= 12) {
            $(".objlist ul").append(
              "<br>plus " + (data.length - count) + " more people."
            );
            break;
          }
        }
